...
const NOISE_LUT_SIZE = 65536;
const noiseLUT = new Float32Array(NOISE_LUT_SIZE);
for (let i = 0; i < NOISE_LUT_SIZE; i++) {
  const u1 = Math.random() || 1e-6;
  const u2 = Math.random();
  const z0 = Math.sqrt(-2.0 * Math.log(u1)) * Math.cos(2.0 * Math.PI * u2);
  noiseLUT[i] = z0 * PHOTOBOOTH_NOISE_STD;
}
let noiseIdx = 0;
...
function applyPhotoboothEffect(imageData, bw = false) {
  ...
  const len = d.length;
  const alpha = PHOTOBOOTH_CONTRAST_ALPHA;
  const beta = PHOTOBOOTH_BRIGHTNESS_BETA;
  
  for (let i = 0; i < len; i += 4) {
    const noise = noiseLUT[noiseIdx++];
    if (noiseIdx === NOISE_LUT_SIZE) noiseIdx = 0;

    if (bw) {
      const gray = 0.299 * d[i] + 0.587 * d[i+1] + 0.114 * d[i+2];
      const v = gray * alpha + beta + noise;
      d[i] = d[i+1] = d[i+2] = v;
    } else {
      d[i]   = d[i]   * alpha + beta + noise;
      d[i+1] = d[i+1] * alpha + beta + noise;
      d[i+2] = d[i+2] * alpha + beta + noise;
    }
  }
  return imageData;
}
